...
C       =============================================
C       POSSIBLE ERRORS
C       1. (2.a.40) POSSIBLE ERROR IN INTERPRETATION OF FLOW CHART
C                   ADVANCE (pg.160)
C       2. (2.b.1)  REMOVED INVERT?? (pg.162)
C       3. (5.b.24) POSSIBLE ERROR IN EXPANSION INTERPRETATION (pg.167)
C       4. (5.c.3) REMOVED 5.c.3 AND 5.c.4 - POSSIBLY INCORRECT?(pg.168)
C       =============================================
C       Start program
C       =============================================
        PROGRAM FR2
C       Setup
        ...
        INTEGER LN,NLJ,NLM,LML(100),JE,KE,CNTA,CNTB,ERROR,UBW,ROW,COL
        ...
        DOUBLE PRECISION SINA,COSA,SCM,SM(100,100),SMR(100,100),AMD(100)
...
C       OPEN READ AND WRITE FILES
        OPEN (UNIT=5,FILE='FR2_INPUT.DAT',STATUS='OLD')
        OPEN (UNIT=6,FILE='FR2_OUTPUT.DAT',STATUS='REPLACE')
C       =============================================
C       FORMATS
C       =============================================
2000    FORMAT(I4)
...
9004    FORMAT(16X,'ANALYSIS OF FRAMED STRUCTURES -- FR2')
...
C       2.a.9
        IF (TS.EQ.4 .OR. TS.EQ.6) THEN
           ...
           SCM4Y=2.0*SCM3Y/L(I)
        END IF
C       2.a.10-2.a.14 REMOVED

C       FR2 -- 3

C       2.a.15 (L2A)
C       FR2 -- 4
        IF (TS.EQ.1) GO TO 507
C       507: L2A1
...
C       2.a.26-30 (L2A9)
C       FR2 -- 5
515     IF (RL(J1).EQ.0) THEN
           ROW=J1-CRL(J1)
           S(ROW,1)=S(ROW,1)+SMD(1,1)
           IF (RL(J2).EQ.0) S(ROW,2)=S(ROW,2)+SMD(1,2)
           IF (RL(K1).EQ.0) THEN
              COL=K1-CRL(K1)-ROW+1
              S(ROW,COL)=SMD(1,3)
           END IF
           IF (RL(K2).EQ.0) THEN
              ...
              S(ROW,COL)=SMD(1,4)
           END IF
           IF (COL.GT.UBW) UBW=COL
        END IF
C       2.a.31-34
        IF (RL(J2).EQ.0) THEN
           ROW=J2-CRL(J2)
           S(ROW,1)=S(ROW,1)+SMD(2,2)
           IF (RL(K1).EQ.0) THEN
              COL=K1-CRL(K1)-ROW+1
              S(ROW,COL)=SMD(2,3)
           END IF
...
C       2.a.35-36
        IF (RL(K1).EQ.0) THEN
           ROW=K1-CRL(K1)
           S(ROW,1)=S(ROW,1)+SMD(3,3)
           IF (RL(K2).EQ.0) S(ROW,2)=S(ROW,2)+SMD(3,4)
        END IF
C       2.a.37
        IF (RL(K2).EQ.0) THEN
           ROW=K2-CRL(K2)
           S(ROW,1)=S(ROW,1)+SMD(4,4)
        END IF
C       2.a.38
C       519: L2
        GO TO 519
C       2.a.39 (L2A10)
C       FR2 -- 6
516     IF (RL(J1).EQ.0) THEN
           ROW=J1-CRL(J1)
           S(ROW,1)=S(ROW,1)+SMD(1,1)
           IF (RL(J2).EQ.0) S(ROW,2)=S(ROW,2)+SMD(1,2)
           IF (RL(J3).EQ.0) THEN
              COL=J3-CRL(J3)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(1,3)
           END IF
           IF (RL(K1).EQ.0) THEN
              COL=K1-CRL(K1)-ROW+1
              S(ROW,COL)=SMD(1,4)
        ...
        IF (RL(J2).EQ.0) THEN
           ROW=J2-CRL(J2)
           S(ROW,1)=S(ROW,1)+SMD(2,2)
           IF (RL(J3).EQ.0) S(ROW,2)=S(ROW,2)+SMD(2,3)
           IF (RL(K1).EQ.0) THEN
              COL=K1-CRL(K1)-ROW+1
              S(ROW,COL)=SMD(2,4)
           END IF
        ...
        IF (RL(K1).EQ.0) THEN
           ROW=K1-CRL(K1)
           S(ROW,1)=S(ROW,1)+SMD(4,4)
           IF (RL(K2).EQ.0) S(ROW,2)=S(ROW,2)+SMD(4,5)
           IF (RL(K3).EQ.0) THEN
              COL=K3-CRL(K3)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(4,6)
           END IF
        END IF
        IF (RL(K2).EQ.0) THEN
           ROW=K2-CRL(K2)
           S(ROW,1)=S(ROW,1)+SMD(5,5)
           IF (RL(K3).EQ.0) S(ROW,2)=S(ROW,2)+SMD(5,6)
        END IF
        IF (RL(K3).EQ.0) THEN
           ROW=K3-CRL(K3)
           S(ROW,1)=S(ROW,1)+SMD(6,6)
        END IF
C       519: L2
        GO TO 519
C       2.a.40 (L2A11)
C       FR2 -- 7
517     IF (RL(J1).EQ.0) THEN
           ROW=J1-CRL(J1)
           S(ROW,1)=S(ROW,1)+SMD(1,1)
           IF (RL(J2).EQ.0) S(ROW,2)=S(ROW,2)+SMD(1,2)
           IF (RL(J3).EQ.0) THEN
           ...
           IF (RL(J4).EQ.0) THEN
              COL=J4-CRL(J4)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(1,4)
           END IF
           IF (RL(J5).EQ.0) THEN
              COL=J5-CRL(J5)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(1,5)
           END IF
           IF (RL(J6).EQ.0) THEN
              COL=J6-CRL(J6)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(1,6)
           END IF
           IF (RL(K1).EQ.0) THEN
              COL=K1-CRL(K1)-ROW+1
              S(ROW,COL)=SMD(1,7)
        ...
        IF (RL(J2).EQ.0) THEN
           ...
           S(ROW,1)=S(ROW,1)+SMD(2,2)
           IF (RL(J3).EQ.0) THEN
              S(ROW,2)=S(ROW,2)+SMD(2,3)
           END IF
           IF (RL(J4).EQ.0) THEN
              COL=J4-CRL(J4)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(2,4)
           END IF
           IF (RL(J5).EQ.0) THEN
              COL=J5-CRL(J5)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(2,5)
           END IF
           IF (RL(J6).EQ.0) THEN
              COL=J6-CRL(J6)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(2,6)
           END IF
           IF (RL(K1).EQ.0) THEN
              COL=K1-CRL(K1)-ROW+1
              S(ROW,COL)=SMD(2,7)
        ...
        IF (RL(J3).EQ.0) THEN
           ...
           S(ROW,1)=S(ROW,1)+SMD(3,3)
           IF (RL(J4).EQ.0) THEN
              S(ROW,2)=S(ROW,2)+SMD(3,4)
           END IF
           IF (RL(J5).EQ.0) THEN
              COL=J5-CRL(J5)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(3,5)
           END IF
           IF (RL(J6).EQ.0) THEN
              COL=J6-CRL(J6)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(3,6)
           END IF
           IF (RL(K1).EQ.0) THEN
              COL=K1-CRL(K1)-ROW+1
              S(ROW,COL)=SMD(3,7)
        ...
        IF (RL(K1).EQ.0) THEN
           ROW=K1-CRL(K1)
           S(ROW,1)=S(ROW,1)+SMD(7,7)
           IF (RL(K2).EQ.0) THEN
              S(ROW,2)=S(ROW,2)+SMD(7,8)
           END IF
           IF (RL(K3).EQ.0) THEN
              COL=K3-CRL(K3)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(7,9)
           END IF
           IF (RL(K4).EQ.0) THEN
              COL=K4-CRL(K4)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(7,10)
           END IF
           IF (RL(K5).EQ.0) THEN
              COL=K5-CRL(K5)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(7,11)
           END IF
           IF (RL(K6).EQ.0) THEN
              COL=K6-CRL(K6)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(7,12)
           END IF
           IF (COL.GT.UBW) UBW=COL
        END IF
        IF (RL(K2).EQ.0) THEN
           ROW=K2-CRL(K2)
           S(ROW,1)=S(ROW,1)+SMD(8,8)
           IF (RL(K3).EQ.0) THEN
              S(ROW,2)=S(ROW,2)+SMD(8,9)
           END IF
           IF (RL(K4).EQ.0) THEN
              COL=K4-CRL(K4)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(8,10)
           END IF
           IF (RL(K5).EQ.0) THEN
              COL=K5-CRL(K5)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(8,11)
           END IF
           IF (RL(K6).EQ.0) THEN
              COL=K6-CRL(K6)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(8,12)
           END IF
           IF (COL.GT.UBW) UBW=COL
        END IF
        IF (RL(K3).EQ.0) THEN
           ROW=K3-CRL(K3)
           S(ROW,1)=S(ROW,1)+SMD(9,9)
           IF (RL(K4).EQ.0) THEN
              S(ROW,2)=S(ROW,2)+SMD(9,10)
           END IF
           IF (RL(K5).EQ.0) THEN
              COL=K5-CRL(K5)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(9,11)
           END IF
           IF (RL(K6).EQ.0) THEN
              COL=K6-CRL(K6)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(9,12)
           END IF
           IF (COL.GT.UBW) UBW=COL
        END IF
        IF (RL(K4).EQ.0) THEN
           ROW=K4-CRL(K4)
           S(ROW,1)=S(ROW,1)+SMD(10,10)
           IF (RL(K5).EQ.0) THEN
              S(ROW,2)=S(ROW,2)+SMD(10,11)
           END IF
           IF (RL(K6).EQ.0) THEN
              COL=K6-CRL(K6)-ROW+1
              S(ROW,COL)=S(ROW,COL)+SMD(10,12)
           END IF
        END IF
        IF (RL(K5).EQ.0) THEN
           ROW=K5-CRL(K5)
           S(ROW,1)=S(ROW,1)+SMD(11,11)
           IF (RL(K6).EQ.0) THEN
              S(ROW,2)=S(ROW,2)+SMD(11,12)
           END IF
        END IF
        IF (RL(K6).EQ.0) THEN
           ROW=K6-CRL(K6)
           S(ROW,1)=S(ROW,1)+SMD(12,12)
        END IF
C       519: L2
        GO TO 519
C       2.b.1 (L2B)
C       FR2 -- 8
520     CALL DECOMPOSEBAND(N,UBW,S)
C       551: L5C1
C        CALL INVERT(N,S)
C       3.a.1
        LN=0
...
C       FR2 -- 11
536     DO 132 J=1,4
        AMD(J)=SMR(J,1)*DJ(J1)+SMR(J,2)*DJ(J2)+SMR(J,3)*DJ(K1)+
     &         SMR(J,4)*DJ(K2)
132     CONTINUE
        WRITE(6,9109) I
C       5.b.11
        DO 133 J=1,4
133     WRITE(6,9133) AML(I,J)+AMD(J)
C       5.b.12-17
        IF (TS.EQ.1) THEN
           IF (RL(J1).EQ.1) AR(J1)=AR(J1)+AMD(1)
           IF (RL(J2).EQ.1) AR(J2)=AR(J2)+AMD(2)
           IF (RL(K1).EQ.1) AR(K1)=AR(K1)+AMD(3)
           IF (RL(K2).EQ.1) AR(K2)=AR(K2)+AMD(4)
        ELSE
           IF (RL(J1).EQ.1) THEN
              AR(J1)=AR(J1)+R(I,1)*AMD(1)+R(I,3)*AMD(2)
           END IF
           IF (RL(J2).EQ.1) THEN
              AR(J2)=AR(J2)+R(I,2)*AMD(1)+R(I,4)*AMD(2)
           END IF
           IF (RL(K1).EQ.1) THEN
              AR(K1)=AR(K1)+R(I,1)*AMD(3)+R(I,3)*AMD(4)
           END IF
           IF (RL(K2).EQ.1) THEN
              AR(K2)=AR(K2)+R(I,2)*AMD(3)+R(I,4)*AMD(4)
           END IF
        END IF
C       5.b.18
C       L5: 550
...
134     CONTINUE
C       5.b.20
        IF (TS.EQ.6) GO TO 555
C       L5B10: 555
C       5.b.21
        DO 135 J=1,6
        AMD(J)=SMR(J,1)*DJ(J1)+SMR(J,2)*DJ(J2)+SMR(J,3)*DJ(J3)+
     &         SMR(J,4)*DJ(K1)+SMR(J,5)*DJ(K2)+SMR(J,6)*DJ(K3)
135     CONTINUE
C       5.b.22
        WRITE(6,9109) I
        DO 141 J=1,6
141     WRITE(6,9133) AML(I,J)+AMD(J)
C       5.b.23
        IF (RL(J1).EQ.1) THEN
           AR(J1)=AR(J1)+R(I,1)*AMD(1)+R(I,4)*AMD(2)+R(I,7)*AMD(3)
        END IF
        IF (RL(J2).EQ.1) THEN
           AR(J2)=AR(J2)+R(I,2)*AMD(1)+R(I,5)*AMD(2)+R(I,8)*AMD(3)
        END IF
        IF (RL(J3).EQ.1) THEN
           AR(J3)=AR(J3)+R(I,3)*AMD(1)+R(I,6)*AMD(2)+R(I,9)*AMD(3)
        END IF
        IF (RL(K1).EQ.1) THEN
           AR(K1)=AR(K1)+R(I,1)*AMD(4)+R(I,4)*AMD(5)+R(I,7)*AMD(6)
        END IF
        IF (RL(K2).EQ.1) THEN
           AR(K2)=AR(K2)+R(I,2)*AMD(4)+R(I,5)*AMD(5)+R(I,8)*AMD(6)
        END IF
        IF (RL(K3).EQ.1) THEN
           AR(K3)=AR(K3)+R(I,3)*AMD(4)+R(I,6)*AMD(5)+R(I,6)*AMD(6)
        END IF
C       L5: 550
        GO TO 550
C       5.b.24 (L5B10)
...
143     WRITE(6,9133) AML(I,J)+AMD(J)
        ...
        END IF
        IF (RL(J4).EQ.1) THEN
           AR(J4)=AR(J4)+R(I,1)*AMD(4)+R(I,4)*AMD(5)+R(I,7)*AMD(6)
        END IF
        IF (RL(J5).EQ.1) THEN
           AR(J5)=AR(J5)+R(I,2)*AMD(4)+R(I,5)*AMD(5)+R(I,8)*AMD(6)
        END IF
        IF (RL(J6).EQ.1) THEN
           AR(J6)=AR(J6)+R(I,3)*AMD(4)+R(I,6)*AMD(5)+R(I,9)*AMD(6)
        END IF
        IF (RL(K1).EQ.1) THEN
           AR(K1)=AR(K1)+R(I,1)*AMD(7)+R(I,4)*AMD(8)+R(I,7)*AMD(9)
        END IF
        IF (RL(K2).EQ.1) THEN
           AR(K2)=AR(K2)+R(I,2)*AMD(7)+R(I,5)*AMD(8)+R(I,8)*AMD(9)
        END IF
        IF (RL(K3).EQ.1) THEN
           AR(K3)=AR(K3)+R(I,3)*AMD(7)+R(I,6)*AMD(8)+R(I,9)*AMD(9)
        END IF
        IF (RL(K4).EQ.1) THEN
           AR(K4)=AR(K4)+R(I,1)*AMD(10)+R(I,4)*AMD(11)+R(I,7)*AMD(12)
        END IF
        IF (RL(K5).EQ.1) THEN
           AR(K5)=AR(K5)+R(I,2)*AMD(10)+R(I,5)*AMD(11)+R(I,8)*AMD(12)
        END IF
        IF (RL(K6).EQ.1) THEN
           AR(K6)=AR(K6)+R(I,3)*AMD(10)+R(I,6)*AMD(11)+R(I,9)*AMD(12)
        END IF
C       L5: 550
        GO TO 550
...
C       5.c.2
        DO 136 K=1,N+NR
136     IF (RL(K).EQ.1) AR(K)=AR(K)-A(K)-AE(K)
C       5.c.3-4 OMITTED
C       FR2 -- 15
C       5.c.5
        IF (TS.LT.3) THEN
...
300     U(I,J)=U(J,I)
        ...
        SUBROUTINE DECOMPOSEBAND(N,UBWW,A)
        IMPLICIT REAL*8 (A-H,O-Z)
        DIMENSION A(100,100)
        INTEGER UBWW
